...
StompClient::StompClient() : connectionHandler(nullptr), stompProtocol(), listenerThread(), mtx(), cv(), isRunning(false), isConnected(false), connection_id() {
	connection_id = stompProtocol.getId();
}
...
void StompClient::handle_logout() {
	if (!isConnected) {
		cout << "Invalid command. You must first login." << endl;
	}
	else {
		isConnected = false;
	}
...
void StompClient::handle_login(const string& host, const string& username, const string& password)
{
	...
	isConnected = true;
	listenerThread = thread([this]  ()  {this->listenToServer();});
}
